...
const fromCSV = (csv, nAttrs = 0) => {
    ...
    const cabecera = lineas[0].split(','); //el nombre de los campos está en la primera fila
    const objectCollection = [];

    
    for (let i = 1; i < lineas.length; i++) { //recorremos todas las líneas
        const contadorLinea = lineas[i].split(',');
        const persona = {};

        let argumentos = cabecera.length;
        if(nAttrs != 0){
            argumentos = nAttrs;
        }
        for (let j = 0; j < argumentos ;j++) { // por cada linea, debemos asignar las propiedades del objeto
            persona[cabecera[j]] = contadorLinea[j];
        }
        
        objectCollection.push(persona); //añadimos el objeto persona al array de objetos
    }
...
const replaceAt = (arr, index, newElement) => {
    ...
    const copy = arr.slice(0,index);    
    copy[index] = newElement;
    copy.push(...arr.slice(index+1,arr.length));
...
const dataEJE3 = [
  ...
  const winnerByYear = (arr, year) => {
    ...
    const filtroAnyo = arr.filter(objeto => objeto.year === year);
    const ganadoresFinales = [];
    if(filtroAnyo.length>0){
        filtroAnyo.sort((a,b)=>b.ranking - a.ranking);
        top3 = filtroAnyo.slice(0,3);
        
        for (i=0;i<3;i++){        
            ganadoresFinales.push(top3[i].name);
        }
    }
...
//   console.log(winnerByYear(dataEJE3, 1998)) // [ 'Douglass', 'Randy', 'Monroe' ]
//   console.log(winnerByYear(dataEJE3, 1999)) // [ 'Graciela', 'Ervin', 'Maxie' ]
//   console.log(winnerByYear(dataEJE3, 2000)) // [ 'Lettie', 'Eddy', 'Emilee' ]
//   console.log(winnerByYear(dataEJE3, 2001)) // [ 'Dino', 'Sam', 'Eleanora' ]
//   console.log(winnerByYear(dataEJE3, 2002)) // [ 'Sean', 'Lang', 'Coleen' ]
//   console.log(winnerByYear(dataEJE3, 2003)) // [ 'Brianna', 'Francene', 'Everett' ]
//   console.log(winnerByYear(dataEJE3, 2004)) // []
...
const collection = [
  ...
  const resultEJE4 = normalize(collection);
  //console.log(resultEJE4);
